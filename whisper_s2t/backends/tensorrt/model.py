...
from .engine_builder.create_trt_model import build_whisper_trt_model
...
class WhisperModelTRT(WhisperModel):
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        self.model_dir = Path(BASE_PATH) / "assets/whisper_models"
        ...
        tokenizer = Tokenizer(tokenizers.Tokenizer.from_file(tokenizer_file), self.model.is_multilingual)  # noqa
        self.model.eot_id = tokenizer.encode("<|endoftext|>")[0]  # noqa
    ...
    def generate_segment_batched(self, features, prompts, seq_lens, seg_metadata, generate_kwargs=None):
        ...
        if self.asr_options["word_timestamps"]:
            text_tokens = [[_t for _t in x[0] if _t <
                            self.tokenizer.eot] + [self.tokenizer.eot] for x in result]
