...
class Tokenizer:
    def __init__(self, tokenizer, multilingual):
        ...
        if self.multilingual:
            self.task_to_token_id = {task: self.tokenizer.token_to_id(
                f"<|{task}|>") for task in _TASKS}
            self.lang_code_to_token_id = {lang: self.tokenizer.token_to_id(
                f"<|{lang}|>") for lang in _LANGUAGE_CODES}
        else:
    ...
    def split_tokens_on_unicode(self, text, tokens):
        ...
        for token in tokens:
            ...
            try:
                replacement_char_index = decoded.index(
                    replacement_char) + unicode_offset
    ...
    def split_tokens_on_spaces(self, text, tokens):
        subwords, subword_tokens_list = self.split_tokens_on_unicode(
            text, tokens)
        words = []
        word_tokens = []
